<template>
	<cover-view class="modal-container"  v-if="showValue" :class="{show: showValue}" @touchmove.stop @click.stop='cancel(2)'>
		<cover-view class="modal-content" @click.stop>
			<cover-view name='title'>
				<cover-view class="modal-title" :class="{'modal-title-padding': !text}" v-if='title'>
					{{title}}
				</cover-view>
			</cover-view>
			<cover-view name='text'>
				<cover-view class="modal-article" v-if="text">
					{{text}}
				</cover-view>
			</cover-view>
			<cover-view name='ipt' v-if="type && type == 1">
				<input class='uni-input search_input' 
				  placeholder='请输入公司名称' />
			</cover-view>
<!-- 			<cover-view name='ipt' v-if="type && type == 2" style="margin-bottom: 40upx;">
				<input class='uni-input search_input' 
				  placeholder='请输入邮箱' v-model="emailIpt" @input="iptChange"/>
				<cover-view v-show="iptRule" class="iptRule">请输入正确的邮箱</cover-view>  
			</cover-view> -->
			<cover-view class="modal-row">
				<cover-view class="modal-col cancel_style" style="color: #A3B1BF;font-weight: normal;" :style="cancelStyle" hover-class="modal-hover" v-if="!noCancel" @click='cancel(1)'>
					{{cancelText}}
				</cover-view>
				<cover-view class="modal-col modal-confirm" :style="confirmStyle" hover-class="modal-hover" @click='confirm'>
					{{confirmText}}
				</cover-view>
			</cover-view>
		</cover-view>
